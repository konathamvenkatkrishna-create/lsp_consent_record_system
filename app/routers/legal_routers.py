...
from app.consent.pdf_utils import generate_pdf
from app.models.consent_master import ConsentMaster

router = APIRouter(prefix="/api/v1/legal", tags=["Legal Docs"])
...
@router.get("/terms")
def get_terms_and_conditions(db: Session = Depends(get_db)):
...
def get_data_consent(db: Session = Depends(get_db)):
    return get_doc_text(db, "Data Consent")
  
@router.get("/credit-bureau")
def get_credit_bureau(db: Session = Depends(get_db)):
    return get_doc_text(db, "Credit Bureau Consent")
 
@router.get("/terms/pdf")
def download_terms_and_conditions(db: Session = Depends(get_db)):
...
def download_privacy_policy(db: Session = Depends(get_db)):
...
def download_data_consent(db: Session = Depends(get_db)):
...
def download_credit_bureau_consent(db: Session = Depends(get_db)):
